...
**Timezone handling**: All timestamps and dates are interpreted as UTC, with no time zone conversions. Because the "billing day" is global and fixed to UTC, travel and device clocks never affect billing. This is simple, consistent, and contractually fair once stated.
...
- `plan_code` is unique and not null.  
- History tracks changes so plan attributes are time-aware.  
- Each dimension includes a default row to ensure facts always join.  

---

### Product catalog (`atlas_catalog_product_info`)

Authoritative list of products and services offered.

| Column       | Type          | Description                                               |
|--------------|---------------|-----------------------------------------------------------|
| product_code | varchar       | Unique product identifier (e.g., PROD-API).               |
| product_name | varchar       | Human-readable product name (e.g., Core API, ETL Engine). |
| category     | varchar       | Product grouping (e.g., platform, data).                  |
| load_ts      | timestamp_ntz | Timestamp the record was loaded.                          |

**Data quality and handling**  
- `product_code` is unique and not null.  
...
**Data quality and handling**  
- `country_code` is unique and not null.  
...
**Data quality and handling**  
- `customer_code` is unique and not null.  
- Default key strategy preserves referential integrity when data is incomplete.  
...
**Data quality and handling**  
- `currency_code` is unique and not null.  
- Default currency member prevents broken joins if the code is missing.  
...
**Data quality and handling**  
- Freshness monitored; uniqueness enforced at grain `(customer, product, plan, date)`.  
- Staging fixes: uppercase codes, coerce dates into valid ranges, filter all-null ghost rows, deduplicate on the natural key keeping the latest by `load_ts` and higher `units_used`, add a default row to support default-key integrity.  
...
- Dates normalized in staging; numeric types enforced.  

---

## Marts (Star Schema)

Dimensions carry surrogate keys generated from business keys. Marts are populated from the refined layer after cleaning, deduplication, history capture, and pricing logic. Default dimension members exist to keep joins intact.
...
| billing_period | varchar | Billing cadence.                |
...
**Grain**: one row per customer × product × plan × report_date.

| Column         | Type          | Description                                                                 |
|----------------|---------------|-----------------------------------------------------------------------------|
| usage_key      | varchar       | Unique surrogate key identifying this daily usage record.                   |
| customer_key   | varchar       | Foreign key to `dim_customer`.                                              |
| product_key    | varchar       | Foreign key to `dim_product`.                                               |
| plan_key       | varchar       | Foreign key to `dim_plan`.                                                  |
| currency_key   | varchar       | Foreign key to `dim_currency`.                                              |
| country_key    | varchar       | Foreign key to `dim_country`.                                               |
| report_date    | date          | UTC calendar day the usage is assigned to.                                  |
...
| unit_price     | number(18,6)  | Official per-unit price from the daily price book.                          |
| billed_amount  | number(18,6)  | Billed amount for overage usage, `overage_units × unit_price`.             |
| load_ts_utc    | timestamp_ntz | When this record was loaded into the warehouse, for lineage and audit.      |
